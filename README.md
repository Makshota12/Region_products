...
flowchart TB
    ...
    F1 --> F11["Ведение реестра продуктов"]
    F1 --> F12["Ведение модели оценки<br/>(домены, критерии, шкалы, веса)"]
    F1 --> F13["Управление сессиями оценки"]
    F1 --> F14["Управление пользователями и ролями"]

    F2 --> F21["Сбор ответов по критериям"]
    F2 --> F22["Верификация ответов экспертом"]
    F2 --> F23["Расчет индекса по доменам"]
    F2 --> F24["Расчет общего индекса зрелости"]
    F2 --> F25["Сравнение продуктов и история индекса"]

    F3 --> F31["Дашборды и графики"]
    F3 --> F32["Генерация PDF-паспорта зрелости"]
    F3 --> F33["Экспорт JSON-отчетов"]
