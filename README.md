...
## 🗣 Сценарии диалога пользователей по ролям

Ниже сценарии оформлены по шаблону, близкому к вашему примеру: общий путь входа в систему, переход в меню и дальнейшие действия по ролям с типовыми командами `Просмотреть / Создать / Изменить / Закрыть`.

### Общий сценарий входа (для всех ролей)

1. **Пользователь:** "Запустить систему."  
   **Система:** "Открыто окно входа."
2. **Пользователь:** "Ввести логин и пароль."  
   **Система:** "Авторизация выполнена, открыта главная страница."
3. **Пользователь:** "Выбрать функцию в меню."  
   **Система:** "Открыт соответствующий модуль с доступными действиями по роли."

### Роль: Администратор системы

1. **Пользователь:** "Открыть модуль 'Продукты'."  
   **Система:** "Доступные действия: Просмотреть, Создать, Изменить, Закрыть."
2. **Пользователь:** "Создать продукт и сохранить карточку."  
   **Система:** "Запись сохранена в реестре продуктов (БД)."
3. **Пользователь:** "Открыть модуль 'Модель оценки' (домены/критерии)."  
   **Система:** "Доступные действия: Просмотреть, Добавить, Изменить, Закрыть."
4. **Пользователь:** "Открыть модуль 'Сессии оценки' и запустить новую сессию."  
   **Система:** "Сессия создана, критерии назначены, статус сессии установлен."
5. **Пользователь:** "Открыть модуль 'Пользователи и роли'."  
   **Система:** "Доступные действия: Просмотреть, Назначить роль, Изменить роль, Закрыть."
6. **Пользователь:** "Закрыть модуль и вернуться в меню."  
   **Система:** "Возврат в главное меню."

### Роль: Эксперт / Аудитор

1. **Пользователь:** "Открыть модуль 'Сессии оценки'."  
   **Система:** "Отображен список сессий, переданных на проверку."
2. **Пользователь:** "Открыть критерий и просмотреть ответ владельца."  
   **Система:** "Показаны балл, метрика, комментарий, файл-доказательство."
3. **Пользователь:** "Запросить уточнение."  
   **Система:** "Статус критерия изменен на 'Запрошены уточнения', комментарий сохранен."
4. **Пользователь:** "Подтвердить корректный критерий."  
   **Система:** "Статус изменен на 'Подтверждено'."
5. **Пользователь:** "Открыть отчет по продукту."  
   **Система:** "Доступные действия: Просмотреть, Сформировать PDF, Закрыть."

### Роль: Владелец продукта

1. **Пользователь:** "Открыть назначенную сессию."  
   **Система:** "Показан список критериев и текущий прогресс заполнения."
2. **Пользователь:** "Заполнить анкету по критериям."  
   **Система:** "Ответы сохранены (балл, метрика, комментарий, вложение)."
3. **Пользователь:** "Отправить анкету на проверку."  
   **Система:** "Сессия передана эксперту, статус обновлен."
4. **Пользователь:** "Получить комментарий эксперта и внести изменения."  
   **Система:** "Изменения приняты, сессия повторно отправлена на верификацию."
5. **Пользователь:** "Открыть результаты."  
   **Система:** "Доступные действия: Просмотреть индекс, Просмотреть графики, Скачать PDF, Закрыть."

### Роль: Наблюдатель

1. **Пользователь:** "Открыть модуль 'Отчеты и аналитика'."  
   **Система:** "Доступные действия: Просмотреть, Закрыть."
2. **Пользователь:** "Просмотреть динамику индекса и сравнение продуктов."  
   **Система:** "Отображены графики и сводные показатели в режиме read-only."
3. **Пользователь:** "Попытаться изменить данные продукта или модели."  
   **Система:** "Операция запрещена (403), изменение недоступно для роли 'Наблюдатель'."

### Визуализация сценариев диалога (Mermaid)

```mermaid
%%{init: {'theme':'default', 'themeVariables': {'background':'#ffffff'}}}%%
flowchart TB
    classDef start fill:#fff4c2,stroke:#c69500,stroke-width:1.8px,color:#5b4400
    classDef step fill:#d8ebff,stroke:#3a7bd5,stroke-width:1.3px,color:#0b3d91
    classDef menu fill:#ffe8b5,stroke:#c69500,stroke-width:1.3px,color:#5b4400
    classDef role fill:#dff5df,stroke:#2d8f4e,stroke-width:1.3px,color:#184d2f

    S0([Старт системы]):::start --> S1[Окно входа]:::step --> S2[Вход в систему]:::step --> S3[Главная страница]:::menu --> S4[Выбор функции]:::menu --> S5[Меню]:::menu

    S5 --> A1["Администратор<br/>Просмотреть / Создать / Изменить / Закрыть<br/>Продукты, модель оценки, сессии, роли"]:::role
    S5 --> E1["Эксперт<br/>Просмотреть / Проверить / Запросить уточнение / Подтвердить / Закрыть"]:::role
    S5 --> O1["Владелец продукта<br/>Просмотреть / Заполнить / Отправить / Изменить по замечаниям / Закрыть"]:::role
    S5 --> N1["Наблюдатель<br/>Просмотреть / Закрыть<br/>(только чтение)"]:::role

    A1 --> R0[Возврат в меню]
    E1 --> R0
    O1 --> R0
    N1 --> R0
    R0 --> X1[Выход из системы]:::step
```
...
Ниже представлена иерархия функций разработанной системы в формате функционального дерева. Диаграмма отражает разделение на основные и служебные функции и может быть использована как иллюстрация функциональной структуры в ВКР.

```mermaid
%%{init: {'theme':'default', 'themeVariables': {'background':'#ffffff'}}}%%
flowchart TD
    classDef root fill:#ffe8b5,stroke:#c69500,stroke-width:2px,color:#5b4400
    classDef group fill:#ffd6d6,stroke:#c0392b,stroke-width:1.5px,color:#7a1d12
    classDef core fill:#d8ebff,stroke:#3a7bd5,stroke-width:1.5px,color:#0b3d91
    classDef proc fill:#dff5df,stroke:#2d8f4e,stroke-width:1.5px,color:#184d2f
    classDef out fill:#fff4c2,stroke:#c69500,stroke-width:1.5px,color:#5b4400
    classDef svc fill:#efe5ff,stroke:#7e57c2,stroke-width:1.5px,color:#45276b

    R["Функции ИС оценки зрелости<br/>цифровых продуктов региона"]:::root

    G1["Основные функции"]:::group
    G2["Служебные функции"]:::group
    R --> G1
    R --> G2

    F1["Управление данными"]:::core
    F2["Обработка и расчет"]:::proc
    F3["Представление и отчетность"]:::out
    S1["Авторизация и аутентификация<br/>(JWT / Google OAuth)"]:::svc
    S2["Журналирование и аудит изменений"]:::svc
    S3["Интеграция с БД PostgreSQL/SQLite"]:::svc
    S4["Мониторинг и логирование<br/>(Prometheus/Grafana/ELK)"]:::svc
    S5["Резервное копирование БД"]:::svc

    %% Горизонтальные связи от групп к функциям (слева направо)
    subgraph MAIN_GROUP[" "]
    direction LR
      G1 --> F1
      G1 --> F2
    ...
    F11["Ведение реестра продуктов"]
    F12["Ведение модели оценки<br/>(домены, критерии, шкалы, веса)"]
    F13["Управление сессиями оценки"]
    F14["Управление пользователями и ролями"]
    F1 --> F11 --> F12 --> F13 --> F14

    F21["Сбор ответов по критериям"]
    F22["Верификация ответов экспертом"]
    F23["Расчет индекса по доменам"]
    F24["Расчет общего индекса зрелости"]
    F25["Сравнение продуктов и история индекса"]
    F2 --> F21 --> F22 --> F23 --> F24 --> F25

    F31["Дашборды и графики"]
    F32["Генерация PDF-паспорта зрелости"]
    F33["Экспорт JSON-отчетов"]
    F3 --> F31 --> F32 --> F33
```
...
В Таблице 3.7 представлены результаты проверки операций управления реестром продуктов и элементами модели оценки. Тестирование подтвердило корректность ключевых CRUD-сценариев: система успешно возвращает список сущностей, создает новые записи и корректно обрабатывает валидационные ошибки при неполных входных данных. Полученные статусы ответов (`200`, `201`, `400/422`) соответствуют ожидаемой семантике REST API и подтверждают устойчивость модуля к типовым пользовательским сценариям.
...
В Таблице 3.8 отражены результаты тестирования ядра прикладной логики - проведения оценочных сессий и процедуры экспертной верификации. Проверки показали, что система корректно создает сессии, сохраняет ответы по критериям и поддерживает полный цикл проверки данных (подтверждение и запрос уточнений) без потери состояния. Это подтверждает работоспособность ролевого workflow и согласованность данных между этапами оценки.
...
В Таблице 3.9 представлены результаты проверки аналитического и отчетного контуров системы. Тестирование подтвердило корректность вычисления интегрального индекса и доменных оценок, а также работоспособность сервисов сравнительного анализа и генерации итогового PDF-паспорта зрелости. Соответствие фактических результатов ожидаемым свидетельствует о завершенности цепочки "расчет -> визуализация -> отчетность" и готовности модуля к практическому использованию.
