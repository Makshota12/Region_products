...
Ниже приведены типовые сценарии взаимодействия пользователей с системой в формате "Пользователь -> Система". Эти сценарии можно использовать как основу для описания пользовательских кейсов в ВКР и для демонстрации на защите.

### Роль: Администратор системы

1. **Пользователь:** "Войти в систему под учетной записью администратора."  
   **Система:** "Авторизация выполнена. Доступны модули управления продуктами, моделью оценки, пользователями и отчетами."
2. **Пользователь:** "Создать новый цифровой продукт."  
   **Система:** "Карточка продукта создана и сохранена в реестре."
3. **Пользователь:** "Настроить модель оценки (домены, критерии, веса)."  
   **Система:** "Изменения модели сохранены в БД и применены к новым сессиям."
4. **Пользователь:** "Запустить новую сессию оценки для продукта."  
   **Система:** "Сессия создана, критерии назначены, участники уведомлены."

### Роль: Эксперт / Аудитор

1. **Пользователь:** "Открыть сессию, переданную на проверку."  
   **Система:** "Отображены ответы, файлы-доказательства и текущий статус критериев."
2. **Пользователь:** "Запросить уточнение по критерию."  
   **Система:** "Статус изменен на 'Запрошены уточнения', комментарий отправлен владельцу."
3. **Пользователь:** "Подтвердить корректный критерий."  
   **Система:** "Статус критерия изменен на 'Подтверждено'."
4. **Пользователь:** "Завершить верификацию и сформировать итоговый отчет."  
   **Система:** "Индексы пересчитаны, PDF-паспорт доступен для выгрузки."

### Роль: Владелец продукта

1. **Пользователь:** "Открыть назначенную оценочную сессию."  
   **Система:** "Показаны критерии, шкалы, комментарии и прогресс заполнения."
2. **Пользователь:** "Заполнить баллы, метрики, комментарии и прикрепить файл."  
   **Система:** "Ответы сохранены, прогресс обновлен."
3. **Пользователь:** "Отправить данные на верификацию."  
   **Система:** "Сессия передана эксперту, статус обновлен."
4. **Пользователь:** "Просмотреть результаты и рекомендации."  
   **Система:** "Доступны дашборды, динамика индекса и итоговый PDF."

### Роль: Наблюдатель

1. **Пользователь:** "Войти в систему и открыть раздел аналитики."  
   **Система:** "Предоставлен доступ только на чтение."
2. **Пользователь:** "Просмотреть текущие индексы и графики по продуктам."  
   **Система:** "Отображены дашборды без возможности редактирования."
3. **Пользователь:** "Попытаться изменить данные модели оценки."  
   **Система:** "Операция запрещена (403), доступ ограничен ролью."

### Визуализация диалоговых сценариев (Mermaid)
...
sequenceDiagram
    ...
    participant N as Наблюдатель
    participant S as Система
    participant DB as База данных

    A->>S: Логин и создание продукта
    S->>DB: Сохранить Product
    A->>S: Запуск сессии оценки
    S->>DB: Сохранить EvaluationSession + AssignedCriterion
    S-->>O: Уведомление о новой сессии

    O->>S: Заполнить ответы по критериям
    S->>DB: Сохранить EvaluationAnswer
    O->>S: Отправить на верификацию
    S-->>E: Запрос на проверку

    E->>S: Запросить уточнение / Подтвердить
    S->>DB: Обновить verification_status
    S-->>O: Вернуть комментарии эксперта

    E->>S: Завершить верификацию
    S->>DB: Пересчитать индексы и сохранить результат
    S-->>A: PDF и дашборд готовы
    S-->>N: Доступ к просмотру результатов (read-only)
```
